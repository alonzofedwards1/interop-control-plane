...
const Dashboard: React.FC = () => {
    ...
    const loadTokenHealth = async () => {
        ...
        try {
            ...
            setTokenHealthError(
                error instanceof Error ? error.message : 'Unable to load token health'
            );
            setTokenHealth(null);
        } finally {
        ...
        <div className="grid">
            ...
            <Card title="System Health">
                ...
                <button
                    ...
                    disabled={!isApiConfigured || loadingHealth}
                >
                ...
                {!isApiConfigured && (
                    <p className="warning">
                        Configure REACT_APP_API_BASE_URL to enable health checks.
                    </p>
                )}
            </Card>
            ...
            <Card title="Token Health">
                <p>Verify token availability before issuing credentials.</p>

                <button
                    type="button"
                    onClick={loadTokenHealth}
                    disabled={!isApiConfigured || loadingTokenHealth}
                >
                ...
                {tokenHealth && (
                    <ul className="status-list">
                        <li>
                            ...
                            {tokenHealth.token_present ? 'Yes' : 'No'}
                        </li>

                        {tokenHealth.token_present && (
                            <>
                                <li>
                                    <strong>Expires At:</strong>{' '}
                                    {tokenHealth.expires_at ?? 'Unknown'}
                                </li>

                                <li>
                                    <strong>Expires In:</strong>{' '}
                                    {tokenHealth.expires_in_seconds ?? '—'} seconds
                                </li>

                                <li>
                                    <strong>Expires Soon:</strong>{' '}
                                    {tokenHealth.expires_soon ? 'Yes' : 'No'}
                                </li>
                            </>
                        )}
                    </ul>
                ...
                {!isApiConfigured && (
                    <p className="warning">
                        Configure REACT_APP_API_BASE_URL to check token readiness.
                    </p>
                )}
            </Card>
            ...
            <Card title="Operational Guidance">
                <ul className="status-list">
                    ...
                    <li>Patient operations return orchestration identifiers only.</li>
                </ul>
            </Card>
        </div>
    );
